...
assert 'strategy == "least_used"' in CREDENTIALS
...
assert "async_step_add_google" in CONFIG
assert "async_step_add_azure" in CONFIG
assert "async_step_manage_credentials" in CONFIG
assert "MINOR_VERSION = 8" in CONFIG
assert "credentials_from_entry(entry)" in INIT
assert "minor_version=8" in INIT
assert "def _provider_order" in HUB
assert "for candidate in self._provider_order(provider, ProviderCapability.STT)" in HUB
assert "for candidate in self._provider_order(provider, ProviderCapability.TTS)" in HUB
